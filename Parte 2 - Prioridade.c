...
struct no *novoProcesso(int chave){
	...
	if(novo){
		...
		novo->prioridade = determinarPrioridade();
		novo->estado = 0;
		return novo;
	}
}
...
int inserirProcesso(int px,int py, struct registrador *processos){
    ...
    if(processos->cabeca==NULL && processos->cauda==NULL){
        //Quando não tem nada da fila
        processos->cabeca = filho;
        processos->cauda = filho;
    }else{
         if((filho->prioridade)>=(processos->cabeca->prioridade)){
            //Verifica se tem que botar na cabeça
            processos->cabeca->ant = filho;
            filho->prox = processos->cabeca;
            processos->cabeca = filho;
        }else if((filho->prioridade)<=(processos->cauda->prioridade)){
            ...
            while(p){
                if(p->prox==NULL){
                    ...
                    if((filho->prioridade)>=(p->prioridade)){
                        //ESTÁ COM ERRO, AJEITAR
                        //E SE NÃO FOR A CABEÇA?
                        p->ant->prox = filho;
                        filho->ant = p->ant;
                        filho->prox = p;
                        p->ant = filho;
...
void imprimirFilaPrioridade(struct registrador *processos){
    struct no *p = processos->cabeca;
    while(p){
        printf("%d-Estado: %d, Prioridade: %d\n", p->identificador,p->estado, p->prioridade);
        p = p->prox;
    }
}
        ...
        if(processoInicial==-1){
            ...
            switch(opcao){
                ...
                case 2:
                    ...
                    imprimirFilaPrioridade(&processos);
                    break;
                case 3:
